...
class WUUtill: NSObject {
    ...
    class func formattedNumber(_ number: String) -> String {
    ...
    class func formatNumber(_ mobileNumberStr: String?) -> String?
    {
        guard var mobileNumber = mobileNumberStr else { return ""}
        
        if mobileNumber.count == 10 || mobileNumber.count == 14 { // mask ########## || (###) ###-####
        ...
        print("Formate mobileNumber == \(mobileNumber)")
...
//        }
    ...
    class func formatLogin(_ mobileNumberStr: String?) -> String? {
...
//            completion(false)
    ...
    class func showAlert_OK_ViewForTabManage(viewController: UIViewController,
        ...
        let alertYESAction = UIAlertAction.init(title: Text.Label.text_Ok,
                                                style: .default,
...
//        IQKeyboardManager.sharedManager().shouldShowToolbarPlaceholder = false
    ...
    class func saveLocalPromotionsModel(_ value : WULocalPromotions){
        let encode = JSONEncoder()
        do {
            let en = try encode.encode(value)
            UserDefaults.standard.set(en, forKey: Text.UDKeys.LocalPromotions)
            UserDefaults.standard.synchronize()
            GlobalShared.localPromotions = value
        }catch{
            Utill.printInTOConsole(printData:"error: \(error.localizedDescription)")
        }
    }
    ...
    class func saveUserModel(_ user : User) {
        let encode = JSONEncoder()
        do {
            let en = try encode.encode(user)
            UserDefaults.standard.set(en, forKey: Text.UDKeys.User)
            UserDefaults.standard.synchronize()
            GlobalShared.user = user
    ...
    class func removeUserModel(){
        ...
        GlobalShared.geolocationFilterData = nil
    }
...
//        }
        ...
        if (image == nil) {
            ...
            manager.imageDownloader?.downloadImage(with: URL(string :path), options: SDWebImageDownloaderOptions(rawValue: 0), progress: nil, completed: { (imageV, Data, errorV, success) in
                if success
                {
...
//        #endif
    ...
    class func showNoInternetView(){
        ...
        let vc = storyBoard.instantiateViewController(withIdentifier: "AllowLocationVC") as! AllowLocationVC
        vc.modalPresentationStyle = .fullScreen

        vc.infoLabelText = Text.Message.noInternetConnection
        vc.buttonOkTitle = Text.Label.text_Try_Again
        guard let rootViewController = AppDel.window?.rootViewController else {return}

        if rootViewController.presentedViewController?.restorationIdentifier != "AllowLocationVC"{
    ...
    class func showErrorView(){
        ...
        vc.infoLabelText = Text.Message.someError
        guard let rootViewController = AppDel.window?.rootViewController else {return}
    ...
    class func showAllowLocationView(){
        let storyBoard : UIStoryboard = UIStoryboard(name: "Home", bundle:nil)
        let vc = storyBoard.instantiateViewController(withIdentifier: "AllowLocationVC") as! AllowLocationVC
        vc.modalPresentationStyle = .fullScreen
    ...
    class func hideAllowLocationView(){
        guard let rootViewController = AppDel.window?.rootViewController else {return}
