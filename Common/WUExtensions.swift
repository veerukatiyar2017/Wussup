...
extension String {
    ...
    var convertUniversalUrlToDeepLink: String{
        
        var deepLink = "https://apps.arvzapp.com"
        let deepLinkDomain = "arvz://"
        let universalUrl = URL(string: self)!
        
        var dict = [String:String]()
        let components = URLComponents(url: universalUrl, resolvingAgainstBaseURL: false)!
        ...
        if dict["EventID"] != "" && dict["EventID"] != "0"{
            deepLink = deepLinkDomain + "event/" + dict["EventID"]!
        }
        
        if dict["LiveCamID"] != "" && dict["LiveCamID"] != "0"{
            deepLink = deepLinkDomain + "livecam/" + dict["LiveCamID"]!
        }
        
        if (dict["VenueID"] != "" && dict["VenueID"] != "0") &&
           (dict["FourSquareID"] != "" && dict["FourSquareID"] != "0"){
            deepLink = deepLinkDomain + "venue/" + dict["VenueID"]! + "/FourSquareID/" + dict["FourSquareID"]!
        }
...
extension Date {
    ...
    static func stringFromCustomDateToUTC(fromDate date:Date, withFormat format:String) -> String {
        let formatter = DateFormatter()
        formatter.dateFormat = format
        formatter.locale = Locale(identifier: "en_US_POSIX")
    ...
    static func eventDateStringInUTC(date: Date)-> String{
        let df = DateFormatter()
        df.timeZone = TimeZone(abbreviation: "UTC")!
...
//        df.locale = Locale(identifier: "en_US_POSIX")
    ...
    func isInNexWeek()-> Bool{
        let modifiedDate = Calendar.current.date(byAdding: .day, value: 14, to: Date())!
        return self <= modifiedDate
    }
    ...
    static func getCombinedDateForDay(startDate :  String , endDate : String ) -> String {
        ...
        if startDay == endDay
        {
            let date1 = Date.stringFromCustomDateToUTC(fromDate: strtDate!, withFormat: "EEEE hh:mm a")
            let date2 = Date.stringFromCustomDateToUTC(fromDate: endedDate!, withFormat: "hh:mm a")
            dateCombine = date1 + " - " + date2
        } else {
            let date1 = Date.stringFromCustomDateToUTC(fromDate: strtDate!, withFormat: "MMM dd, hh:mm a")
            let date2 = Date.stringFromCustomDateToUTC(fromDate: endedDate!, withFormat: "MMM dd, yyyy hh:mm a")
            dateCombine = date1 + " - " + date2
        }
        //        if (components.day! >= 1)
        //        {
    ...
    static func getCombinedDateForDayAbdRemoveZero(startDate :  String , endDate : String ) -> String {
        ...
        if startDay == endDay
        {
            let date1 = Date.stringFromCustomDateToUTC(fromDate: strtDate!, withFormat: "EEEE")
            var date2 = Date.stringFromCustomDateToUTC(fromDate: strtDate!, withFormat: "hh:mm a")
            if date2.first == "0" {
                date2.removeFirst()
            }
            var date3 = Date.stringFromCustomDateToUTC(fromDate: endedDate!, withFormat: "hh:mm a")
            if date3.first == "0" {
                date3.removeFirst()
            }
            dateCombine = date1 + " " + date2  + " - " + date3
        } else {
            let date1 = Date.stringFromCustomDateToUTC(fromDate: strtDate!, withFormat: "MMM dd")
            var date2 = Date.stringFromCustomDateToUTC(fromDate: strtDate!, withFormat: "hh:mm a")
            if date2.first == "0" {
                date2.removeFirst()
            }
            let date3 = Date.stringFromCustomDateToUTC(fromDate: endedDate!, withFormat: "MMM dd, yyyy")
            var date4 = Date.stringFromCustomDateToUTC(fromDate: endedDate!, withFormat: "hh:mm a")
            if date4.first == "0" {
                date4.removeFirst()
            }
            
            dateCombine = date1 + " " + date2 + " - " + date3 + " " + date4
        }
...
extension UIColor {
    ...
    static var LightYellowAppColor: UIColor {
         return  UIColor(red: 255.0.colorRange, green: 203.0.colorRange, blue: 5.0.colorRange, alpha: 1.0)
     }
    
    static var DarkYellowAppColor: UIColor {
           return  UIColor(red: 170.0.colorRange, green: 133.0.colorRange, blue: 44.0.colorRange, alpha: 1.0)
       }
    
    
    static var DarkGreyAppColor: UIColor {
           return  UIColor(red: 74.0.colorRange, green: 74.0.colorRange, blue: 74.0.colorRange, alpha: 1.0)
       }
