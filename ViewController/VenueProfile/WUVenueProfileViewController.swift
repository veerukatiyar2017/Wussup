...
class WUVenueProfileViewController: UIViewController {
    ...
    @IBOutlet weak var soundOnOffButtonOutlet                       : UIButton!
    ...
    private func initialInterfaceSetUp() {
        ...
        self.tableViewVenueProfile.contentInset = UIEdgeInsetsMake(0, 0, 0, 0);
...
//        self.call_api_GetLocationKeyOfCity()
    ...
    private func prepareViewForVenueProfileDetail() {
        ...
        //print("VenueDistance! in WUVenueProfileViewController \(self.venueProfileDetail.VenueDistance)")
        ...
        if (self.venueProfileDetail.VenueAtOpen == "" && self.venueProfileDetail.VenueAtClose == "") {
            ...
            if self.venueProfileDetail.IsVenueOpen.toBool() == true {
                ...
                self.labelOpenWith.text = Text.Label.text_Open
            } else {
                self.labelOpenWith.textColor = .RedColor
                self.labelOpenWith.text = Text.Label.text_Closed
            }
...
//        }
        ...
        if self.venueProfileDetail.VenueOpenHours.count == 5 {
            self.viewTimeSheetHeight.constant = 125
        }
    ...
    private func setupVenueVideoContainer(){
        self.imageviewVenueFeaturedImage.image = UIImage(named: "NullState_CoverPhoto")
        
        if self.venueProfileDetail.VenueFeaturedImageType == "" || self.venueProfileDetail.VenueFeaturedImageType == "Image"{
        ...
        } else if self.venueProfileDetail.VenueFeaturedImageType == "Video"{
            self.soundOnOffButtonOutlet.isHidden = false
            
            if self.player != nil {
                if self.tableViewVenueProfile.contentOffset.y < self.viewVideoContainer.frame.size.height{
                    self.player?.play()
                }
            }else{
        ...
        }else if self.venueProfileDetail.VenueFeaturedImageType == "LiveCamURL"{
    ...
    func callWS_getVenueProfileDetail(isFromFavorite : Bool){
        ...
        if self.fourSquareVenueId == "" || self.sponsoredVenuID == "" || self.latitude == "" || self.longitude == ""
            {
            if venue is WUVenue {
                fourSquareVenueId = (venue as! WUVenue).FourSquareVenueID
                sponsoredVenuID = (venue as! WUVenue).SponsoredVenuID
                latitude = (venue as! WUVenue).VenueLattitude
                longitude = (venue as! WUVenue).VenueLongitude
                ...
                if self.venueProfileDetail != nil {
                    ...
                    latitude = self.venueProfileDetail.VenueLattitude
                    longitude = self.venueProfileDetail.VenueLongitude
                }
            }
        }
        ...
        WEB_API.call_api_GetVenueProfileDetail(user: GlobalShared.user,
                                               ...
                                               sponsoredVenuID: sponsoredVenuID,
    ...
    private func setUpForVenueProfileDetail(){
        ...
        self.soundOnOffButtonOutlet.isHidden = true
    ...
    @IBAction func soundOnOffButtonAction(_ sender: UIButton) {
        guard self.player != nil else {return}
        self.player!.isMuted =  self.player!.isMuted == true ? false:true
        let backgroundImageName = self.player!.isMuted == true ? "MuteIcon" : "soundIcon"
        self.soundOnOffButtonOutlet.setBackgroundImage(UIImage(named: backgroundImageName), for: .normal)
    }
...
//                vc.callWS_getHomeVenueList()
    ...
    @IBAction func buttonTimeSheetWithStatusAction(_ sender: Any)
    {
        if self.viewTimeSheet.isHidden == true {
            //timesheet will not be visible if opened
            if let viewOpenTime = self.viewDetail.viewWithTag(venueOpenNowTag) {
                let viewOpenTimeYOffset = ((self.viewDetail.frame.origin.y + viewOpenTime.frame.size.height - 25) -
                    self.view.frame.size.height + self.viewTimeSheet.frame.size.height )
...
extension WUVenueProfileViewController : UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if scrollView.contentOffset.y > self.viewVideoContainer.frame.size.height{
            self.player?.pause()
        }else {
            self.player?.play()
        }
...
extension WUVenueProfileViewController : UICollectionViewDelegate , UICollectionViewDataSource ,UICollectionViewDelegateFlowLayout{
    ...
    private func callForVenue(cell : WUCategoryCollectionCell, forIndexPath indexPath : IndexPath){
        if self.venueProfileDetail.VenuePhone == "" {
            ...
            if let url = URL(string: "tel://\(Utill.formatNumber(self.venueProfileDetail.VenuePhone)!)"),
    ...
    private func favoriteVenueForCell(cell : WUCategoryCollectionCell, forIndexPath indexPath : IndexPath){
        ...
        self.callWS_favoriteVenue(isFavorite: self.isFavoriteVenue, withCompletion:
            { (success) in
                if success == true {
                    self.labelFavoriteCount.text = "(\(self.venueProfileDetail.VenueFavoriteCount))"
                    self.buttonStarFavourite.isSelected = false
    ...
    func venueShareInfo() {
        WEB_API.call_api_GetShareLinkForVenueOrEvent(strVenueID: self.venueProfileDetail.SponsoredVenuID,strFourSquareVenueID : self.venueProfileDetail.FourSquareVenueID, strEventID: "", strLiveCamID: "") { (response, success, message) in
            if success == true {
                ...
                let shareAll : [Any] = [WUShareActivityItemProvider(placeholderItem: self.imageviewVenueFeaturedImage.image) as Any,
                                        ...
                                        WUShareActivityItemProvider(placeholderItem: "\nI found this venue on ArvzApp and wanted to share it!") as Any,
                                         WUShareActivityItemProvider(placeholderItem: "\nVenue Name : \(self.venueProfileDetail.VenueName)") as Any,
                                        WUShareActivityItemProvider(placeholderItem: "\n\(shareLink ?? "https://apps.arvzapp.com/")") as Any,
                                        WUShareActivityItemProvider(placeholderItem: coverPhotoURL) as Any]
...
extension WUVenueProfileViewController : WUVenueRateDelegate{
 ...
 // MARK: - Video Player methods
    extension WUVenueProfileViewController{
    ...
    private func playContinousVideo() {
        if self.venueProfileDetail != nil {
            self.playVideo(withUrl: self.venueProfileDetail.VideoList)
        }
    }
...
*/
                    ...
                    self.playerController.view.frame = self.viewVideoContainer.bounds
                    self.playerController.showsPlaybackControls = false
                    self.playerController.updatesNowPlayingInfoCenter = false
                    
                    DispatchQueue.main.asyncAfter(deadline: .now() + 4) {
                        self.viewVideoContainer.addSubview(self.playerController.view)
                        self.player?.play()
    ...
    private func playPlaceholderVideo() {
        ...
        var urlVideosPlaceholder =  self.venueProfileDetail.VenueFeaturedImage
        //urlVideosPlaceholder = "http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4"  // TEST!!!
...
// MARK: - LiveCam Player methods
   extension WUVenueProfileViewController{

private func loadURLTOPlayer(){

    if let urlVideo = URL(string: self.venueProfileDetail.VenueFeaturedImage) {
         let avplayerItem = AVPlayerItem(url: urlVideo)
         self.player = AVPlayer(playerItem: avplayerItem)
         self.player?.rate = 1.0;
         self.player?.isMuted = true
