...
//        Utill.loadBeeAnimationForView(view: self.view)
    ...
    private func callWS_Login(){
        let userName = self.dictTextField[Text.DictKeys.email]!.contains("@") ? self.dictTextField[Text.DictKeys.email]! : Utill.formatNumber(self.dictTextField[Text.DictKeys.email]!)
        
        WEB_API.call_api_Login(userName: userName!, passWord: self.textFieldPassword.text!) { (response, success, message) in
            if success{
    ...
    private func checkValidation(){
        ...
        let finalUserName =  Utill.formatLogin(self.dictTextField[Text.DictKeys.email]!)
...
extension WULoginViewController : UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if let textRange = Range(range, in: textField.text!) {
            ...
            if textField == self.textFieldEmail {
                // phone
                if (string.isNumeric || string == "") && (Utill.formatLogin(finalText)?.isNumeric)! {
...
//                    }
                   ...
                   // textField.text = Utill.formatLogin(textField.text)
                }
...
//                    self.textFieldEmail.resignFirstResponder()
                    ...
                    Utill.setPlaceHolderTextAndColor(textfield: self.textFieldEmail,
                                                     placeHolderString: Text.TextField.text_EmailOrMobileError,
                                                     placeHolderTextColor: .SearchBarYellowColor)
                }
...
//                  self.textFieldPassword.resignFirstResponder()
                    ...
                    Utill.setPlaceHolderTextAndColor(textfield: self.textFieldPassword,
                                                     placeHolderString: Text.TextField.text_PasswordError,
                                                     placeHolderTextColor: .SearchBarYellowColor)
