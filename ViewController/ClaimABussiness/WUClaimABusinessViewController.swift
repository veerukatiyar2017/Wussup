...
class WUClaimABusinessViewController: UIViewController {
    ...
    @IBOutlet private weak var promoteYourBizLabel              : UILabel!
    @IBOutlet private weak var otherLabels                      : UILabel!
    @IBOutlet private weak var freeForDaysLabel                 : UILabel!
    @IBOutlet private weak var noCreditCardLabel                : UILabel!
...
//    @IBOutlet private weak var labelBusinessOperation           : UILabel!
    ...
    @IBOutlet private weak var viewSeperatorAndButton           : UIView!
    ...
    private func setUpUIView() {
        self.tableViewClaimABusiness.tableHeaderView?.frame = CGRect(x: 0,
                                                                     y: 0,
                                                                     width: self.tableViewClaimABusiness.frame.size.width,
                                                                     height: 459.0.propotionalHeight)
        ...
        self.viewSeperatorAndButton.isHidden            = true
...
//        self.labelBusinessOperation.textColor           = UIColor.white
       ...
       // self.promoteYourBizLabel.textColor              = UIColor.CABOrangeColor
        
        let promoteAttributedString = NSMutableAttributedString(string:"PROMOTE ",
                                                  attributes:[.font: UIFont.ProximaNovaBold(23.0.propotional) as Any])
        
        let yourAttributedString = NSAttributedString(string:"your ",
                                                  attributes:[.font: UIFont.italicSystemFont(ofSize: 23)])
        
        let bizAttributedString = NSAttributedString(string:"BIZ ",
                                                  attributes:[.font: UIFont.ProximaNovaBold(23.0.propotional) as Any])
        
        let includesAttributedString = NSAttributedString(string:"includes:",
                                                  attributes:[.font: UIFont.italicSystemFont(ofSize: 23)])
        
        promoteAttributedString.append(yourAttributedString)
        promoteAttributedString.append(bizAttributedString)
        promoteAttributedString.append(includesAttributedString)
        self.promoteYourBizLabel.attributedText         = promoteAttributedString
        
        
        let freeAttributedString = NSMutableAttributedString(string:"• FREE ",
                                                             attributes:[.font: UIFont.ProximaNovaBold(18.0.propotional) as Any])
        
        let daysAttributedString = NSAttributedString(string:"for 60 Days ",
                                                      attributes:[.font: UIFont.ProximaNovaMedium(18.0.propotional) as Any])
        
        freeAttributedString.append(daysAttributedString)
        self.freeForDaysLabel.attributedText            = freeAttributedString
        self.freeForDaysLabel.textColor                 = UIColor.btnForgotPwdGreenColor

        self.otherLabels.textColor                      = UIColor.white
        self.noCreditCardLabel.textColor                = UIColor.btnForgotPwdGreenColor
    ...
    private func callWS_getSuggestionList(searchText : String){
        ...
        WEB_API.call_api_GetClaimSuggestions(user: Utill.getUserModel()!, strSearchText: searchText){ (response, status, message) in
            ...
            if status == true {
                ...
                self.buttonOK.isHidden = false
                self.buttonClaimABusiness.isHidden = true
                self.viewSeperatorAndButton.isHidden = false
            }
...
//                    self.textFieldSearch.resignFirstResponder()
            ...
            self.dictTextField[Text.DictKeys.email] = Utill.formatLogin(finalText)
