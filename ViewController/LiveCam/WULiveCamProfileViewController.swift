...
class WULiveCamProfileViewController: UIViewController,UIViewControllerTransitioningDelegate {
    ...
    var liveCamCoverImage =  UIImageView()
    
    //MARK: - Load Methods
    override func viewDidLoad() {
        super.viewDidLoad()
        
        self.collectionAds.register(UINib(nibName: Utill.getClassNameFor(classType:WUHomeSliderCollectionCell()),
                                          bundle: nil),
    ...
    {
        self.liveCamCoverImage.sd_setImage(with: URL(string: self.liveCamVenue.ImageURL))
        ...
        self.tableViewLiveCamProfile.tableHeaderView?.frame = CGRect(x: 0,
                                                                     y: 0,
                                                                     width: self.tableViewLiveCamProfile.frame.width,
    ...
    func callWS_GetLiveCamDetail()
    {
        ...
        WEB_API.call_api_GetLiveCamDetail(strLiveCamID: self.liveCamID) { (response, success, message) in
            ...
            if success == true {
                let venuePromotionsData = try! JSONSerialization.data(withJSONObject: response?["Promotions"].dictionaryObject ?? [:],
                                                                      options: [])
                let localPromotions:WULocalPromotions = try! JSONDecoder().decode(WULocalPromotions.self, from: venuePromotionsData)
                self.venuePromotions = localPromotions.VenuePromotions
                
                let liveCamVenueData = try! JSONSerialization.data(withJSONObject: response?["LiveCamsURL"].dictionaryObject ?? [:],
                                                                   options: [])
                self.liveCamVenue = try! JSONDecoder().decode(WUVenueLiveCams.self, from: liveCamVenueData)
                print("liveCamVenue = \(self.liveCamVenue)")
//                if self.venuePromotions.count == 0 {
    ...
    private func manageViewDescription(isShow : Bool){
        ...
        if isShow == true {
            ...
            self.tableHeaderView.frame = CGRect(x: 0,
                                                y: 0,
                                                width: self.tableViewLiveCamProfile.frame.size.width,
                                                height: viewheaderHeightForDescription) //490.0
            self.tableViewLiveCamProfile.reloadData()
            ...
            if self.venuePromotions.count == 0 {
                ...
                self.tableHeaderView.frame = CGRect(x: 0,
                                                    y: 0,
                                                    width: self.tableViewLiveCamProfile.frame.size.width,
                                                    height: viewheaderHeightForNoDescriptionWithNoBanner) //390.0
            } else {
                self.cnstCollectionViewHeight.constant = 100
                self.tableHeaderView.frame = CGRect(x: 0,
                                                    y: 0,
                                                    width: self.tableViewLiveCamProfile.frame.size.width,
                                                    height: viewheaderHeightForNoDescriptionWithBanner) //490.0
            }
        }
    }
...
//        }
    ...
    @IBAction func buttonShareAction(_ sender: Any) {
        ...
        WEB_API.call_api_GetShareLinkForVenueOrEvent(strVenueID: "",strFourSquareVenueID : "", strEventID: "", strLiveCamID: self.liveCamID) { (response, success, message) in
            if success == true{
                ...
                let shareAll : [Any] = [WUShareActivityItemProvider(placeholderItem: self.liveCamCoverImage.image) as Any,
                                        WUShareActivityItemProvider(placeholderItem: "Check out this Live Cam on ArvzApp!") as Any,
                                        WUShareActivityItemProvider(placeholderItem: "\nI found this Live Cam on ArvzApp and wanted to share it!") as Any,
                                        WUShareActivityItemProvider(placeholderItem:"\nLiveCam Name : \(self.liveCamVenue.Name)") as Any,
                                        WUShareActivityItemProvider(placeholderItem: "\n\(shareLink ?? "https://apps.arvzapp.com/")") as Any]
      
                
                let activityViewController = UIActivityViewController(activityItems: shareAll, applicationActivities: nil)
                activityViewController.setValue("Check out this Live Cam on ArvzApp!", forKey: "Subject")
    ...
    @objc func updateImage() {
        if self.venuePromotions.count > 0{
            ...
            if self.currentVisibleImageIndex >= self.venuePromotions.count {
...
extension WULiveCamProfileViewController : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if  self.venuePromotions[indexPath.row].VenueStatusID == 1 {
           // FirebaseManager.sharedInstance.ad_herobanner_GUID_ios(parameter:nil, homeBanner:self.venuePromotions[indexPath.row])
