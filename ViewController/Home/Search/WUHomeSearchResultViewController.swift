...
class WUHomeSearchResultViewController: UIViewController {
    ...
    private func addPlaceholderToArrayFilterSearchVenues() {
        ...
        if self.arrayFilterSearchVenues.count > 0 {
            self.showResult()
            for i in 0..<self.arrayFilterSearchVenues.count {
                // I find when they start simple accounts
                if self.arrayFilterSearchVenues[i].IsPremiumVenue == false {
    ...
    //MARK: - Hidden Label Of No Result Found
    ...
    private func callWS_getSearchedVenueList(){
        ...
        { (response, status, message) in
            if status == true {
                ...
                //Utill.showAlertView(viewController: self, message: "Server error.\nThis venue is not found.")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .searchResult:
            if self.placeholderCardAdsIndex == indexPath.row { // create placeholder by indexPath.row
                ...
                //print("VENUE: \(cell.venue.VenueName) VenuePhone: \(cell.venue.VenuePhone)")
                _ = cell.venue.VenuePhone.count == 0 ? (cell.buttonCall.isHidden = true) : (cell.buttonCall.isHidden = false)
                return cell
            }
