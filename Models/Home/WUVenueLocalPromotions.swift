...
class WUVenueLocalPromotions: NSObject, Decodable {
    ...
    required init(from decoder: Decoder) throws {
        ...
        do {
            if let NotificationID = try values.decodeIfPresent(Int.self, forKey: .NotificationID) {
                self.NotificationID = NotificationID
            }
        } catch DecodingError.typeMismatch {
            Utill.printInTOConsole(printData:"type mismatched in WUVenueLocalPromotions for NotificationID ")
        }
    }
}
