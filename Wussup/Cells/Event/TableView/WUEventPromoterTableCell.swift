...
//        self.imageViewPromoter.sd_setShowActivityIndicatorView(true)
 //       self.imageViewPromoter.sd_setIndicatorStyle(.white)
        self.imageViewPromoter.sd_imageIndicator = SDWebImageActivityIndicator.white
...
//            }
//        }
        self.imageViewPromoter.sd_setImage(with: URL(string:self.eventPromotor.ImageURL), placeholderImage: #imageLiteral(resourceName: "placeholder"), options: .refreshCached) { (image, error, cacheType, url) in
            if  (self.delegate != nil) &&  image != nil{
                self.delegate?.expandableImageTableCell(innerCell: self, didLoadImage: image!)
            }
                }
