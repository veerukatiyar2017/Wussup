...
class WUEventCollectionCell: UICollectionViewCell {
    ...
    private func initialInterfaceSetUp() {
        self.viewDescriptionContainer.dropShadow(color: .lightGray, opacity: 1.0, offSet: CGSize(width: 0.0, height: 4.0), radius: 4.0, scale: true)
    ...
    private func setCellDetail() {
        ...
        self.imageViewCoverPhoto.sd_imageIndicator = SDWebImageActivityIndicator.white
//        self.imageViewCoverPhoto.sd_setShowActivityIndicatorView(true)
//        self.imageViewCoverPhoto.sd_setIndicatorStyle(.white)
        self.imageViewCoverPhoto.sd_setImage(with:URL(string:self.event.ConverPhotoURL) , placeholderImage: UIImage(named:"placeholder.png" ))
        self.labelMainTitle.text = event.Name
        self.labelDescription.text = self.event.Description
        self.labelDayTime.text = Date.stringFromCustomDate(fromDate: Date.dateObjectNotUTC(dateStr: self.event.StartDate)!, withFormat: "hh:mm a")
    }
