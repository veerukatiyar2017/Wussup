...
//            self.imageViewIcon.sd_setIndicatorStyle(.white)
            self.imageViewIcon.sd_setImage(with: URL(string: self.foodCategory.ImageURL), placeholderImage: #imageLiteral(resourceName: "placeholder"), options: .refreshCached, completed : nil)
        }
        else {
            self.imageViewIcon.isHidden = true
            self.viewIcon.isHidden = true
