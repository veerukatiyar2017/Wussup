...
//        self.viewClaimABusinessAlert.isHidden = true
        ...
        if self.venueLiveCamsOrArtEntmt is WUVenueLiveCams {
            ...
            self.imageViewTitle.sd_imageIndicator = SDWebImageActivityIndicator.white
...
//            self.imageViewTitle.sd_setIndicatorStyle(.white)
            ...
            if coverImage.count > 0 {
                self.imageViewTitle.sd_setImage(with: URL(string: coverImage[0].SquareImage), placeholderImage:#imageLiteral(resourceName: "NullState_CoverPhoto") , options: .refreshCached)
            }else {
