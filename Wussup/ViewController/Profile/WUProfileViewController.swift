...
//        self.reloadDataForProfile()
    ...
    func reloadDataForProfile() {
        ...
        profileMobileModel.profilePlaceHoderValue = self.user.Mobile//Utill.formatNumber("1234567899")!//self.user.MobileNumber
        ...
        for objCat in self.user.FavoriteCategories{
            let profileInterestModel = ProfileCellDataModel.init()
            profileInterestModel.profileCellType = .cellTypeImageLabel
            profileInterestModel.profilePlaceHoder = objCat.SelectedImageURL
            profileInterestModel.profilePlaceHoderValue = objCat.WussupName
        ...
        if self.user.ImageURL != "" {
            self.imageUserProfile.sd_imageIndicator = SDWebImageActivityIndicator.white
...
//            self.imageUserProfile.sd_setIndicatorStyle(.white)
        ...
        let strIDSepratedByCommas = self.user.FavoriteCategories.map { $0.ID }.joined(separator: ",") //joined(separator: ",")
    ...
    func isProfileEdited() -> Bool {
        ...
        let userCategory = self.user.FavoriteCategories.map { $0.ID }.joined(separator: ",")
    ...
    // MARK: - Webservice Methods
        func callWS_GetUserProfile(){
            WEB_API.call_api_GetUserProfile(){ (response, success, message) in
                if success{
    ...
    func callWS_EditUserProfile(completionHandler : @escaping (Bool) -> Void) {
        ...
        if self.isProfileEdited()
        {
            ...
            WEB_API.call_api_EditUserProfile(username: dic[Text.DictKeys.profileUsername]!, imageUrl: dic[Text.DictKeys.profileUserImageUrl]!, email: dic[Text.DictKeys.profileEmail]!, mobile: self.dicEditedUserProfile[Text.DictKeys.profileMobile]!, birthDate: dic[Text.DictKeys.profileBirthday]!,allowNotification:dic[Text.DictKeys.profileFavNotification]! , categories: dic[Text.DictKeys.profileCategory]!){ (response, success, message) in
    ...
    private func pickUpDate(_ textField : UITextField){
        ...
        if let birthday = textField.text, birthday.count > 0, let strDate = birthday.dateFromCustomString(withFormat: "MMMM dd, yyyy") {
    ...
    func checkValidation() -> Bool {
        ...
        if self.dicEditedUserProfile[Text.DictKeys.profileEmail] == "" && self.dicEditedUserProfile[Text.DictKeys.profileMobile] == "" {
            return false
...
extension WUProfileViewController : UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if profileDataModel.profileCellType == .cellTypeOnlyLabel {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: Utill.getClassNameFor(classType: WUProfileEditDoneTableCell()), for: indexPath)as! WUProfileEditDoneTableCell
            cell.backgroundColor = .DarkGrayColor
            cell.delegate = self
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: Utill.getClassNameFor(classType: WUProfileTitleSubTitleTableViewCell()), for: indexPath)as! WUProfileTitleSubTitleTableViewCell
            cell.backgroundColor = .DarkGrayColor
            ...
            if profileDataModel.profileCellDataType != .cellDataTypeCity{
                ...
                if profileDataModel.profileCellDataType == .cellDataTypeEmail{
                    ...
                    cell.textFldSubtitle.text = Utill.arrangeUSFormat(strPhone: profileDataModel.profilePlaceHoderValue)
            ...
            cell.imageViewCategory.sd_imageIndicator = SDWebImageActivityIndicator.white
//            cell.imageViewCategory.sd_setShowActivityIndicatorView(true)
//            cell.imageViewCategory.sd_setIndicatorStyle(.white)
            cell.imageViewCategory.sd_setImage(with: URL(string:profileDataModel.profilePlaceHoder), placeholderImage:#imageLiteral(resourceName: "placeholder") , options: .refreshCached)
            return cell
        }
    }
...
extension WUProfileViewController : WUProfileFavoriteSwitchTableCellDelegate {
    func profileFavoriteSwitchTableCell(cell: WUProfileFavoriteSwitchTableCell, valueChange switchToggle: UISwitch) {
        ...
        if cellModel.profileCellDataType == .cellDataTypeNotification {
            ...
            let strIDSepratedByCommas = self.user.FavoriteCategories.map { $0.ID }.joined(separator: ",")
            WEB_API.call_api_EditUserProfile(username: self.user.UserName, imageUrl: self.user.ImageURL, email: self.user.Email, mobile: self.user.Mobile, birthDate: self.user.Birthdate,allowNotification:dicEditedUserProfile[Text.DictKeys.profileFavNotification]! , categories:strIDSepratedByCommas){ (response, success, message) in
                if success{
                    let data = try! JSONSerialization.data(withJSONObject: response?["UserProfile"].dictionaryObject ?? [:], options: [])
                    let user = try! JSONDecoder().decode(WUUser.self, from: data)
                    user.Token = self.user.Token
