...
//            cell.imageViewShare.sd_setIndicatorStyle(.white)
            cell.imageViewShare.sd_setImage(with: URL(string: self.arrShareOptions[indexPath.section].shareImage), placeholderImage:#imageLiteral(resourceName: "WussupBg") , options: .refreshCached)
            if indexPath.section == 0 {
                cell.imageViewShare.sd_setImage(with: URL(string: self.arrShareOptions[indexPath.section].shareImage), placeholderImage:#imageLiteral(resourceName: "NullState_CoverPhoto") , options: .refreshCached)
            }
            cell.delegate = self
        ...
        if indexPath?.section == self.arrShareOptions.count-1 {
            ...
            WEB_API.call_api_GetShareLinkForVenueOrEvent(strVenueID: self.venueShareDetail.SponsoredVenuID,strFourSquareVenueID : self.venueShareDetail.FourSquareVenueID, strEventID: "", strLiveCamID: "") { (response, success, message) in
                if success == true{
                    ...
                    let shareAll : [Any] = [WUShareActivityItemProvider(placeholderItem: coverPhotoURL) as Any,WUShareActivityItemProvider(placeholderItem: shareText ?? Text.Message.msg_Deeplinking_Share) as Any,WUShareActivityItemProvider(placeholderItem: shareLink ?? "https://apps.arvzapp.com/") as Any,WUShareActivityItemProvider(placeholderItem:"Venue Name : \(self.venueShareDetail.VenueName)") as Any]
                    let activityViewController = UIActivityViewController(activityItems: shareAll, applicationActivities: nil)
                    let activityTypeCloud = UIActivityType.init("com.apple.CloudDocsUI.AddToiCloudDriven")
