...
class WUEventSelectedCategoryViewController: UIViewController {
    ...
    private func callWS_getEventListForSelectedCategory(){
        WEB_API.call_api_GetEventList(user: GlobalShared.user, categoryId: (self.selectedEventCategory?.ID)!, date: Date(), searchData: SearchVenueAndEventData()) { (response, success, message) in
