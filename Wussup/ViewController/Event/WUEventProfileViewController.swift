...
//    private var arrMyeventList : [WUMyEventList] =  []
    ...
    private func prepareViewForEventProfileDetail(){
        self.imageviewEventFeaturedImage.sd_imageIndicator = SDWebImageActivityIndicator.white
...
//        self.imageviewEventFeaturedImage.sd_setIndicatorStyle(.white)
        self.imageviewEventFeaturedImage.sd_setImage(with: URL(string: self.eventProfileDetail.ConverPhotoURL), placeholderImage:#imageLiteral(resourceName: "placeholder") , options: .refreshCached)
        ...
        self.labelDate.text = Date.getCombinedDateForDay(startDate: self.eventProfileDetail.StartDate, endDate: self.eventProfileDetail.EndDate)
    ...
    private func callWS_getEventDetail(eventId : String){
        WEB_API.call_api_GetEventDetail(eventId: eventId) { (response, success, message) in
...
extension WUEventProfileViewController : UICollectionViewDelegate , UICollectionViewDataSource ,UICollectionViewDelegateFlowLayout{
    ...
    private func callForVenue(cell : WUCategoryCollectionCell, forIndexPath indexPath : IndexPath){
        if self.eventProfileDetail.Phone == "" {
            ...
            if let url = URL(string: "tel://\(Utill.formatNumber(self.eventProfileDetail.Phone)!)"), UIApplication.shared.canOpenURL(url) {
    ...
    private func shareVenue(cell : WUCategoryCollectionCell, forIndexPath indexPath : IndexPath){
        ...
        WEB_API.call_api_GetShareLinkForVenueOrEvent(strVenueID: "", strFourSquareVenueID : "", strEventID: self.eventProfileDetail.ID, strLiveCamID: "") { (response, success, message) in
            if success == true{
                ...
                let shareAll : [Any] = [WUShareActivityItemProvider(placeholderItem: coverPhotoURL) as Any,WUShareActivityItemProvider(placeholderItem: shareText ?? Text.Message.msg_Deeplinking_Share) as Any,WUShareActivityItemProvider(placeholderItem: shareLink ?? "https://apps.arvzapp.com/") as Any,WUShareActivityItemProvider(placeholderItem:"Event Name : \(self.eventProfileDetail.Name)") as Any,WUShareActivityItemProvider(placeholderItem: "Event StartDate : \(finalStartDate)\nEvent EndDate : \(finalEndDate) ") as Any]
