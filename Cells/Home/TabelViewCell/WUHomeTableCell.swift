...
//                        }
    ...
    func createTapGesture() {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(showClaimBussinessVC(_:)))
        tapGesture.numberOfTapsRequired = 1
        noVenuePlaceholder.isUserInteractionEnabled = true
        noVenuePlaceholder.addGestureRecognizer(tapGesture)
    }
    ...
    @objc func showClaimBussinessVC(_ sender : UITapGestureRecognizer) {
        if let delegate = self.delegate{
            delegate.showClaimBussinessVC()
        }
    }
}
