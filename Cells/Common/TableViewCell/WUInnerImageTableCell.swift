...
//        self.isFromAdmission = false
        ...
        if self.venueProfileListInnerImage is WUVenueHours {
            self.imageViewCell.sd_setImage(with: URL(string:(self.venueProfileListInnerImage as! WUVenueHours).ImageURL), placeholderImage:#imageLiteral(resourceName: "placeholder") , options: [SDWebImageOptions.cacheMemoryOnly])
        }else if self.venueProfileListInnerImage is WUVenueMenus {
            self.buttonAddToCalendar.isHidden = true
            self.imageViewCell.sd_setImage(with: URL(string:(self.venueProfileListInnerImage as! WUVenueMenus).MenuImageURL), placeholderImage:#imageLiteral(resourceName: "placeholder") , options: [SDWebImageOptions.cacheMemoryOnly])
        }else if self.venueProfileListInnerImage is WUVenueLiveMusic {
            self.imageViewCell.sd_setImage(with: URL(string:(self.venueProfileListInnerImage as! WUVenueLiveMusic).ImageURL), placeholderImage:#imageLiteral(resourceName: "placeholder") , options: [SDWebImageOptions.cacheMemoryOnly])
        }else if  self.venueProfileListInnerImage is WUEvent
        {
             self.imageViewCell.sd_setImage(with: URL(string:(self.venueProfileListInnerImage as! WUEvent).ConverPhotoURL), placeholderImage:#imageLiteral(resourceName: "placeholder") , options: [SDWebImageOptions.cacheMemoryOnly])
        }
        else{
//            if self.venueProfileListInnerImage is WUMoreEvent {
            if self.isFromAdmission == true {
                ...
                self.imageViewCell.sd_setImage(with: URL(string:(self.venueProfileListInnerImage as! WUMoreEvent).ImageURL), placeholderImage:#imageLiteral(resourceName: "placeholder") , options: [SDWebImageOptions.cacheMemoryOnly])
        }
    }
