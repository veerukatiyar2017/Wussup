...
extension WUEventTabelCell : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: Utill.getClassNameFor(classType:WUEventCollectionCell()),
                                                      for: indexPath)as! WUEventCollectionCell
        cell.delegate = self

        let startDate = self.dateWiseEvents.Events[indexPath.row].StartDate
        let endDate = self.dateWiseEvents.Events[indexPath.row].EndDate
        
        let startDateDay = Date.dateObject(dateStr: startDate)?.dayAsString()
        let endDateDay = Date.dateObject(dateStr: endDate)?.dayAsString()
        
        let startDateMonth = Date.dateObject(dateStr: startDate)?.monthAsString().uppercased()
        let endDateMonth = Date.dateObject(dateStr: endDate)?.monthAsString().uppercased()
        
        if let theStartDate = Date.dateObject(dateStr: startDate), theStartDate.isToday() {
            cell.viewDescriptionContainer.backgroundColor = UIColor.white //UIColor.LiveCamYellowColor
        } else {
        ...
        if startDateDay == endDateDay{
            ...
            cell.centerMonthLabel.isHidden = true
            cell.labelDate.text = startDateDay
            cell.labelDate.textAlignment = .center
            cell.labelMonth.text = startDateMonth
            cell.labelMonth.textAlignment = .center
        }else if startDateDay != endDateDay && startDateMonth == endDateMonth{
            cell.endDateStackView.isHidden = true
            cell.centerMonthLabel.isHidden = false
            cell.labelDate.text =  "\(startDateDay!)-\(endDateDay!)"
            cell.labelDate.textAlignment = .center
            cell.centerMonthLabel.text = startDateMonth
            ...
            cell.labelMonth.text = startDateMonth
            cell.lebelEndDate.text = endDateDay
            cell.lebelEndDate.textAlignment = .left
            cell.lebelEndMonth.text = endDateMonth
            cell.labelMonth.textAlignment = .left
        }
